...
formInfo 两个功能 按照和你的沟通

1. 存储前面用来展示表单
2. 存储快照数据

训练模型的 
1.   存储了表单和表单数据 (只存储了步骤二，三，四) 其实就是 getDynamicForm的数据
2.   至于  存储快照数据。 可以理解也是存储了 getDynamicForm 的模板数据

在添加预测服务
1. 实现了模型 的 formInfo 也就是快照数据（仅仅只是用步骤三）。按照快照数据来讲也是没有像模型名称这样的字段
2.  此时会调用一下 getDynamicForm 获取另外一份 表单

3. 按照之前的沟通 提交的时候  formInfo 就存储了 (模型步骤二，三，四和新增)

...
当调用 main 函数时，f64 类型的值 3.4 被分配到堆栈上，而不与任何变量相关联。
当函数 f 被调用时，堆栈中分配了四个指针，一个用于参数 p，一个用于三个变量 a、b 和 c 中的每一个。
当执行该函数的第一条语句时，堆中分配了一个 f64 对象，并使用表达式 * p 的值进行初始化，即 3.4
当函数的第二条语句执行时，会在堆中分配并初始化一个包含三个 i32 值的数组，并使用它的地址来初始化 b 变量。第三条语句打印 a 和 b 引用的值。用于 Box 对象的解引用操作符与用于简单引用的操作符相同。 c 变量无法在此处打印，因为它尚不可见。在此语句之后，b 的范围立即结束，因此 b 引用的数组从堆中释放，释放它使用的空间，使其可用于其他分配。
这最后一步很重要。当 b 退出其范围时，它不再可用；并且自动地，它所引用的堆对象被释放。
当函数的第四条语句执行时，在堆中分配并初始化一个布尔对象，其地址用于初始化c变量。可能是这样的布尔对象与堆中的前一个数组重叠，因为数组使用的空间已被释放。第五个语句打印 a 和 c 引用的值。同样对于框，就像简单的参考一样，星号是可选的并且被省略。 b 变量无法在此处打印，因为它不再可见。在此语句之后，a 和 c 的范围立即结束，因此它们引用的对象从堆中释放。这也是函数执行的结束，因此四个指针也从堆栈中释放。
最后，主函数结束，所以 3.4 未命名对象从堆栈中释放。
这里比较没有意义，但是值得注意的是，如果在同一个作用域中声明了多个变量，它们会以声明的相反顺序退出它们的作用域。在我们的示例中，a 已在 c 之前声明，因此 a 在 c 之后退出其范围。这会导致 a 引用的对象在 c 引用的对象之后从堆中释放。这是相当合理的，考虑到它是一个堆栈，所以顶部的项目总是下一个要删除的。
请注意，这里没有调用释放函数。实际上，Rust 语言及其标准库没有可调用的释放函数。这可以防止忘记调用它们，或者为同一个对象调用它们两次。

### Similarity with C and C++ 与 C 和 C++ 的相似性

即使在 C 和 C++ 中，你也可以使用堆。在 C 语言中，您可以使用 malloc、calloc 和 realloc 函数在堆中分配缓冲区，并使用 free 函数来释放先前分配的缓冲区。此外，在 C++ 语言中，您可以使用 new 和 new[] 运算符分别在堆中分配一个对象或对象数组；并且您有 delete 和 delete[] 运算符来分别释放由 new 或 new[] 运算符分配的内容。
实际上，Rust Box< T > 泛型类型与上述所有类型的堆分配完全不同。但自 2011 年以来，在标准 C++ 中存在一种与 Box< T > 非常相似的类型：它是 unique_ptr< T > 类模板。它是一个所谓的智能指针，就像 Box< T > 一样，在堆中分配一个对象，并在它退出其作用域时自动释放它。

### Boxing and Unboxing  装箱和拆箱
因此，对于给定的 T 泛型类型， Box< T > 和 &T 都是引用类型。让我们看看他们如何互动：
...
堆栈将包含三个对象：数字 7，由 a 变量表示；和两个指针，由 a_box 和 a_ref 变量表示。第二行中声明的指针 a_box 仅在第五行初始化，而其他变量在声明它们的相同语句中初始化。
两个指针变量都是类型注释的，但是这样的注释是可选的并且可以被删除，因为它们的类型可以从它们的使用中推断出来。但是，它们表明 a_box 是一个智能指针，而 a_ref 是一个哑指针，这意味着 a_box 负责分配和释放其引用对象，而 a_ref 不负责分配或释放其引用对象。
在两个打印宏调用中，三个星号是可选的，可以删除
在第五行和第六行，两个指针被赋值。但是对于 a_box，它是一个初始化，所以变量不必是可变的；但是，a_ref 已经被初始化，因此，要重新分配它必须使用 mut 子句声明。
第三行只是将a_ref变量值设置为a变量的地址。但是第五行做了一些更复杂的事情。它在堆中分配一个 i32 对象，使用表达式 a + 2 的值（即值 9）初始化该对象，然后将 a_box 变量值设置为该对象的地址
第六行，将a_box的值（不是a_box所引用的值），也就是一个指针，复制到变量a_ref中；换句话说，哑指针指向智能指针指向的同一个对象。最后的打印语句证实了这一点。但是，这个赋值不能简单的 a_ref = a_box;，因为这两个变量的类型不同，而且语句 a_ref = a_box as &i32;将是非法的。相反，使用 * 取消引用，然后使用 & 引用的技巧允许我们将 Box 转换为引用。更好的说法是，它允许我们获取盒子引用的对象的地址
请注意，反转操作是非法的：a_box = &* a_ref;。实际上，表达式 &* a_ref 仍然是 &i32 类型，因此不能将它分配给 Box< i32 > 类型的变量。
最后，在程序结束时，首先 a_ref 退出了它的作用域，什么也不做；然后 a_box 从它的作用域中存在，从堆中释放它的引用对象；然后 a 退出其范围，什么也不做；最后这三个对象从堆栈中释放。
...
```
这里 a_box 是可变的，而 a_ref 是不可变的。
last-but-one 语句重新分配框。从概念上讲，这会导致先前引用的对象（其值为 9）从堆中释放，并在堆中分配一个新对象。新对象的值取自 a_ref 引用的值，即 7。

### Register Allocation  寄存器分配

在汇编语言中，有时也在 C 语言中，使用处理器寄存器分配的概念。在 Rust 语言中，没有这个概念，因为它将代码限制在特定的目标硬件架构上。但是，代码优化器可以将堆栈分配对象的位置移动到处理器寄存器中，只要程序的结果行为是等效的。因此，在源代码级别看起来是堆栈分配的对象，在机器代码级别可能会变成寄存器分配的对象。这当然取决于目标架构，因为目标处理器的寄存器越多，可以分配到寄存器中的变量就越多。
这种寄存器分配通常与程序员无关。但是如果你使用源码级调试器检查一个高度优化的程序的内存内容，你会发现一些堆栈分配的变量已经消失了。因此，在调试时，您应该指示编译器生成未优化的可执行代码，除非您想直接调试机器代码。